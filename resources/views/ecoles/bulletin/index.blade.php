...
<script>
    ...
    const MOIS_PAR_PERIODE = {
        trimestre: { 1: ['Octobre','Novembre','Décembre'], 2: ['Janvier','Février','Mars'], 3: ['Avril','Mai','Juin'] },
        semestre:  { 1: ['Octobre','Novembre','Décembre','Janvier','Février'], 2: ['Mars','Avril','Mai','Juin'] }
    };
    ...
    document.getElementById('bulletinForm').addEventListener('submit', function (e) {
        ...
        if (!niveauId || !classeId || !eleveId || !periodeNumero) {
        ...
        fetchJson(`/${SLUG}/bulletin/data?` + new URLSearchParams({
            annee_scolaire: anneeScolaire, niveau_id: niveauId, classe_id: classeId,
            eleve_id: eleveId, periode_type: periodeType, periode_numero: periodeNumero
        }))
        .then(data => {
            try {
                renderBulletin(data);
    ...
    function renderBulletin(data) {
        ...
        // Génération du QR code (infos de vérification du bulletin)
        // Contenu volontairement court : un QR code a une capacité limitée (~976 caractères
        // au niveau de correction par défaut). On encode juste l'essentiel, pas de texte verbeux.
        if (typeof QRCode === 'undefined') {
            ...
            try {
                new QRCode(document.getElementById('bulletinQr'), {
                    ...
                    colorDark: '#1d2939',
                    colorLight: '#ffffff',
                    typeNumber: 0, // 0 = laisser la librairie choisir automatiquement la taille adaptée
                    correctLevel: QRCode.CorrectLevel.L // niveau de correction bas = plus de capacité de texte
                });
            } catch (qrErr) {
                console.error(qrErr);
                showToast('QR code non généré (contenu trop long). Le reste du bulletin fonctionne normalement.', 'warning');
            }
        }
