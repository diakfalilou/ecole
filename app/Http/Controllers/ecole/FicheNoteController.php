...
class FicheNoteController extends Controller
{
     ...
     * - Si "matiere_id" est fourni : une seule fiche pour cette matière (quel que soit le niveau)
     * - Sinon :
    ...
    public function getFiche(Request $request, $slug)
    {
        ...
        // "combiner" ne s'applique que si aucune matière spécifique n'a été choisie
        $combiner = !$request->filled('matiere_id')
            && (str_contains($nomNiveau, 'maternelle') || str_contains($nomNiveau, 'primaire'));
        ...
        $matieres = DB::table('tblclassematiere as cm')
            ...
            ->when($request->filled('matiere_id'), fn ($q) => $q->where('m.id', $request->matiere_id))
            ->select('m.id', 'm.nom')
            ->orderBy('m.nom')
            ->get();

        abort_if($request->filled('matiere_id') && $matieres->isEmpty(), 404, 'Matière introuvable pour cette classe.');
